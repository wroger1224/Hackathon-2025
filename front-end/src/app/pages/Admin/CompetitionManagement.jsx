...
const CompetitionManagement = () => {
        ...
        const updatedTeams = competition.teams.map(team => {
            if (team.id === editingTeamId) {
                return {
                    ...
                    members: editedTeam.members.map(id => mockUsers.find(u => u.id === parseInt(id)))
                };
            }
            return team;
        });
    ...
    const handleAddTeam = () => {
        ...
        const captain = mockUsers.find(user => user.id === parseInt(newTeam.captain));
        const members = selectedMembers.map(id => mockUsers.find(user => user.id === parseInt(id)));
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl font-bold">Manage Competition: {competition.name}</h1>
                {!isEditing && (
                    <button
                    ...
                    <div className="bg-white p-6 rounded-lg shadow">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center">
                                ...
                                />
                                <label className="ml-2 block text-sm text-gray-900">Active Competition</label>
                            </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-blue-50 p-4 rounded-lg">
                            <h3 className="text-lg font-semibold mb-2">Last Week's Activity</h3>
                            <div className="space-y-2">
                                <p>Total Participants: {competition.participationStats.lastWeek.totalParticipants}</p>
                                <p>Active Participants: {competition.participationStats.lastWeek.activeParticipants}</p>
                                <p>Participation Rate: {competition.participationStats.lastWeek.participationRate}%</p>
                            </div>
                        </div>
                        <div className="bg-green-50 p-4 rounded-lg">
                            <h3 className="text-lg font-semibold mb-2">Team Participation</h3>
                            <div className="space-y-2">
                                {competition.participationStats.lastWeek.teamStats.map(teamStat => {
                                    const team = competition.teams.find(t => t.id === teamStat.teamId);
                                    return (
                                        <div key={teamStat.teamId} className="flex justify-between items-center">
                                            <span>{team.name}:</span>
                                            <span>{teamStat.activeMembers}/{teamStat.totalMembers} active</span>
                                        </div>
                                    );
                                })}
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >
                                        ...
                                        {mockUsers
                                            .filter(user => !selectedMembers.includes(user.id.toString()))
                                            .map(user => (
                                                <option key={user.id} value={user.id}>
                                                    {user.name}
                                                </option>
                                ...
                                <div className="mt-2">
                                    {selectedMembers.map(memberId => {
                                        const member = mockUsers.find(u => u.id === parseInt(memberId));
                                        return (
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            >
                                                ...
                                                {mockUsers
                                                    .filter(user => !editedTeam.members.includes(user.id.toString()))
                                                    .map(user => (
                                                        <option key={user.id} value={user.id}>
                                                            {user.name}
                                                        </option>
