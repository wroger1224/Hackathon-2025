...
try {
    ...
    // 4. Insert Users first (before teams since teams reference users)
    const insertUser = db.prepare(`
        INSERT INTO User (
            UserID, FirstName, LastName, Email, OptInForMessages, 
            Age, Weight, ActivityLevel, HeightInInches, CurrentTeamID, RoleID
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    // 5. Insert Teams (now that users exist)
    const insertTeam = db.prepare(`
        INSERT INTO Team (TeamID, TeamName, TeamCaptainID, CompetitionID, Points, Rank)
        VALUES (?, ?, ?, ?, ?, ?)
    `);
    // Teams will be created with captain IDs
    insertTeam.run(1, 'Cardiac Crusaders', 1, 1, 0, 1);
    insertTeam.run(2, 'Heart Heroes', 7, 1, 0, 2);
    insertTeam.run(3, 'Pulse Protectors', 13, 1, 0, 3);
    insertTeam.run(4, 'Beat Masters', 19, 1, 0, 4);
